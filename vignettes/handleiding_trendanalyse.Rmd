...
is dat de kwaliteit van de data op orde is. De selectie vindt op de volgende manier plaats:
  ...
  d) selecteer alle waarnemingen over de periode 1998-2024; 
...
*verwijder* is een veld om aan te geven welke metingen afgekeurd
worden. Dit veld is 0 als de stof niet afgekeurd is, iedere waarde >0
geeft aan dat de meting is afgekeurd.
...
krijgen. Standaard wordt de tabel drempelwaarden gebruikt, maar er kan ook een
andere tabel worden gebruikt. Zie ook het voorbeeld bij *lijst met drempelwaarden*.
...
Standaard wordt de tabel drempelwaarden gebruikt, maar er kan ook een
andere tabel worden gebruikt.

```{r}
dw(param = "no3", gwl = "NLGW0001")

# voorbeeld andere tabel
drempelwaarden2 <- drempelwaarden %>% 
  mutate(no3 = ifelse(gwbident == "NLGW0001", 60, no3))

dw(param = "no3", gwl = "NLGW0001", data = drempelwaarden2)
```
...
kolom detectielimiet aangegeven met een 1. Met de functie `replaceDL`
kunnen deze waarden vervangen worden door andere waarden
zoals 0.5 maal de waarde, 0 of met een missing value.
...
Voor het toetsen van de normoverschrijding wordt de gemeten waarde
...
```{r}
# selecteer no3 voor grondwaterlichaam NLGW0003 ondiep
d <- grondwaterdata %>% 
...
doorlopen te worden voor de ondiepe en diepe filters:
  ...
  de dataset voor trendanalyse KRW beoordeeld worden. De putfilters die minder 
  dan 10x zijn bemonsterd, dienen te worden afgekeurd.
...
```
  ...
  b) *verdeling waarnemingen 1998-2024* van de reeksen van waarnemingen
  per stof per putfilter dienen ten minste een waarneming in de 
  ...
  twee waarneming in de derde planperiode KRW (2016-2024). Als een reeks niet 
  voldoet aan dit criterium, dient de reeks te worden afgekeurd. 
  ...
  vervolgens worden afgekeurd. Zie onderstaand script voor een voorbeeld:
...
# Periodes met overlap
resultaat <- meetverdeling(d, jaren_overlap = 1)
...
resultaat <- resultaat %>% spread(key = tijdsperiode, value = aantal_metingen, fill = 0)
...
for (i in unique(grondwaterdata$putfilter)) {
  ...
  resultaat <- resultaat %>%
    bind_rows(data.frame(put = i, meetverdeling(d, jaren_overlap = 1),
                         stringsAsFactors = FALSE)) 
}
# Voor een beter overzicht wordt de data van long naar wide format omgezet
resultaat <- resultaat %>% spread(key = tijdsperiode, value = voldoende_metingen)
knitr::kable(head(resultaat))
...
```
  ...
  c) *aantal waarnemingen > rapportagegrens* aantal reeksen met >4 waarnemingen
  boven rapportagegrens. Als een reeks niet voldoet aan dit criterium, dient de 
  reeks te worden afgekeurd. 
  
  Voor het uitvoeren van de statistische toets dienen ten minste 5 metingen in 
  een reeks aanwezig te zijn. Dit betekent dat de trendanalyse alleen kan worden
  uitgevoerd als er ten minste 4 waarnemingen (>RG) en 1 rapportagegrens
  in een reeks zitten. Dit is een arbitrair criterium. Bij het overzicht van de 
  bemonsteringsfrequentie is het minimum aantal metingen al bepaald. In deze 
  stap worden de reeksen geselecteerd die aan het criterium van minimaal 4 
  waarnemingen >RG voldoen.
  
  Een overzicht per put per stof per diepte kan verkregen worden met onderstaand 
  voorbeeld. Reeksen met onvoldoende waarnemingen > rapportagegrens kunnen
  vervolgens worden afgekeurd. Zie onderstaand script voor een voorbeeld:
  
```{r}
# Aantal waarnemingen boven rapportagegrens
d <- grondwaterdata %>% 
  group_by(putfilter, parameter, diepte) %>% 
  summarise(n_boven_rg = sum(detectielimiet == 0, na.rm = TRUE)) 

# Putfilters per stof per diepte met kolom afgekeurd obv minimaal 4 waarnemingen
resultaat <- d %>% 
  mutate(afgekeurd_rg = n_boven_rg < 4)

knitr::kable(resultaat)

```
  
  d) *overzicht statistieken* maak overzicht van de statistieken van 
...
van reeksen van waarnemingen in stap 4d.

#### *Toelichting goedkeuren en afkeuren van reeksen*
In stap 3a, 3b en 3c worden reeksen afgekeurd op basis van het niet halen van de 
criteria. 

Het goedkeuren en afkeuren van reeksen gebeurt op basis van arbitraire 
criteria. Met deze arbitraire criteria wordt onderscheid gemaakt in (1) 
goedgekeurde reeksen waarvoor mogelijk trends kunnen worden benoemd op basis van 
het aantal metingen, de verdeling in de tijd en het aantal metingen boven de 
rapportagegrens en (2) reeksen waarvoor waarschijnlijk het aantal metingen niet 
voldoende zijn om trends te kunnen bepalen of er te veel rapportagegrenzen 
aanwezig zijn in een reeks.

In tabel 4 wordt een overzicht gegeven van het aantal reeksen en het aantal 
goed- en afgekeurde reeksen. Deze informatie dient in stap 4 te worden gebruikt 
bij de interpretatie van de trendresultaten en in stap 6 bij het komen tot een 
uitspraak op niveau van een grondwaterlichaam. 

  e) *overzicht gekeurde reeksen* maak overzicht van de goedgekeurde reeksen. Om 
  te komen tot een overzicht van goedgekeurde reeksen moet een overzicht van het 
  aantal metingen, de meetverdeling en het aantal metingen boven rapporatgegrens 
  gecombineerd worden. Putten die in één of meer van de drie overzichten staan 
  worden afgekeurd.
    
  Een overzicht van het aantal metingen kan verkregen worden met onderstaand 
  voorbeeld. Putten met minder dan 10 metingen worden in dit voorbeeld 
  afgekeurd.
  
  Een overzicht per put per stof per diepte kan verkregen worden met onderstaand 
  voorbeeld. Reeksen met onvoldoende waarnemingen > rapportagegrens kunnen
  vervolgens worden afgekeurd. Zie onderstaand script voor een voorbeeld (in het
  voorbeeld staat meetverdeling voor meetfrequentie):

```{r}
# Overzicht Meetfrequentie
d <- grondwaterdata %>%
  group_by(grondwaterlichaam, diepte, parameter) %>%
  count(putfilter)

overzicht_meetfrequentie <- d %>% 
  mutate(afgekeurd_freq = n <10) %>% 
  select(grondwaterlichaam, putfilter, parameter, diepte, afgekeurd_freq)
```
  
  Een overzicht van de meetverdeling kan verkregen worden met onderstaand 
  voorbeeld. Onder normale omstandigheden moeten er 2 metingen in de 3e 
  planperiode (2016-2024) aanwezig zijn. De voorbeeld data bevat geen metingen 
  na 2016. Daardoor worden alle reeksen afgekeurd. Voor meer variatie in het 
  uiteindelijke overzicht wordt daarom het voorbeeld met jaren overlap gebruikt. 

```{r}
# Overzicht Meetverdeling
# Periodes zonder overlap
d <- grondwaterdata %>% 
  group_by(grondwaterlichaam, diepte, parameter, putfilter) %>% 
  nest() %>% 
  mutate(tijdelijk = purrr::map(data, ~meetverdeling(.))) %>% 
  select(-data) %>% 
  unnest(tijdelijk) %>% 
  pivot_wider(names_from = tijdsperiode, values_from = c(aantal.metingen), values_fill = 0)

overzicht_meetverdeling <- d %>% 
  mutate(afgekeurd_meetverdeling = ifelse(`1e planperiode KRW (2004-2009)` <1 | 
                                            `2e planperiode KRW (2010-2015)` <2 |
                                            `3e planperiode KRW (2016-2024)` <2,
                                          TRUE, FALSE)) %>% 
  select(grondwaterlichaam, putfilter, parameter, diepte, afgekeurd_meetverdeling)

# Alternatief
# Periodes jaren overlap
d <- grondwaterdata %>% 
  group_by(grondwaterlichaam, diepte, parameter, putfilter) %>% 
  nest() %>% 
  mutate(tijdelijk = purrr::map(data, ~meetverdeling(., jaren_overlap = 2))) %>% 
  select(-data) %>% 
  unnest(tijdelijk) %>% 
  pivot_wider(names_from = tijdsperiode, values_from = c(voldoende.metingen), values_fill = 0)

overzicht_meetverdeling_overlap <- d %>% 
  mutate(afgekeurd_meetverdeling = ifelse(`1e planperiode KRW (2004-2009)` == FALSE | 
                                            `2e planperiode KRW (2010-2015)` == FALSE |
                                            `3e planperiode KRW (2016-2024)` == FALSE,
                                          TRUE, FALSE)) %>% 
  select(grondwaterlichaam, putfilter, parameter, diepte, afgekeurd_meetverdeling)
```

  Een overzicht van het aantal waarnemingen boven de rapportagegrens kan 
  verkregen worden met onderstaand voorbeeld.

```{r}
# Overzicht Waarnemingen boven rapportagegrens
# Aantal waarnemingen boven rapportagegrens
d <- grondwaterdata %>% 
  group_by(grondwaterlichaam, putfilter, parameter, diepte) %>% 
  summarise(n_boven_rg = sum(detectielimiet == 0, na.rm = TRUE)) 

# Putfilters per stof per diepte met kolom afgekeurd obv minimaal 4 waarnemingen
overzicht_waarnemingen_boven_rg <- d %>% 
  mutate(afgekeurd_rg = n_boven_rg < 4) %>% 
  select(grondwaterlichaam, putfilter, parameter, diepte, afgekeurd_rg)
```

  Een overzicht van de gekeurde reeksen kan verkregen worden met onderstaand 
  voorbeeld. Voor het voorbeeld is het overzicht van de meetverdelinge met jaren
  overlap gebruikt omdat dit een gevarieerder overzicht geeft.
  

```{r}
# Tabel 4
# Samenvoegen resultaten uit de voorgaande voorbeelden
resultaat <- left_join(overzicht_meetfrequentie, overzicht_meetverdeling_overlap) %>% 
  left_join(., overzicht_waarnemingen_boven_rg)

# Met behulp van alle resultaten wordt een kolom afgekeurd gemaakt
putfilterselectie <- resultaat %>% 
  mutate(afgekeurd = ifelse(afgekeurd_rg == TRUE | 
                              afgekeurd_freq == TRUE | 
                              afgekeurd_meetverdeling == TRUE, 
                            TRUE, FALSE)) %>% 
  select(grondwaterlichaam, putfilter, parameter, diepte, afgekeurd)

# Overzicht zoals tabel 4 van werkwijze
resultaat <- putfilterselectie %>% 
  group_by(grondwaterlichaam, parameter, diepte) %>% 
  summarise(totaal_aantal_reeksen = n(), 
            aantal_goedgekeurde_reeksen = sum(afgekeurd == FALSE), 
            aantal_afgekeurde_reeksen = sum(afgekeurd == TRUE))
...
In stap 4 wordt voor de reeksen die in stap 3 zijn geselecteerd de 
trendtoets daadwerkelijk uitgevoerd. Dit is ten minste voor de grondwaterlichamen
waarin een 75% normoverschrijding is aangetroffen (zie stap 2). 

De trendtoets in stap 4 bestaat uit het uitvoeren van een statistische trendtoets
op putfilterniveau (stap 4b), een correctie voor 'vals positieven' (stap 4c), 
indien nodig, een visuele inspectie van de reeksen en een expert beoordeling 
(stap 4d) en het verzamelen van de statistieken van het eindresultaat in iteratie 
2 (stap 4e). Voordat deze stappen worden toegelicht, wordt eerst ingegaan op de 
...
Om zicht te krijgen op de invloed van rapportagegrenzen op het resultaat (zie ook
stap 3d) dient de trendanalyse volgens iteratie 1 en 2 te worden uitgevoerd.
In iteratie 1 wordt de trendtoets uitgevoerd op de dataset waarbij <
rapportagegrens wordt geherdefinieerd als de helft van de rapportagegrens. In
iteratie 2 wordt de trendtoets nogmaals uitgevoerd waarbij < rapportagegrens wordt
geherdefinieerd als 0.

Als de resulaten van iteratie 1 en 2 niet verschillen dan hebben de 
rapportagegrenzen in de dataset geen invloed op het resultaat van de trendtoets. 
Verdere actie is niet nodig. Het resultaat van iteratie 1 kan worden gebruikt.

Als de resultaten van iteratie 1 en 2 wel verschillen dan dienen de reeksen op 
filterniveau visueel te worden beoordeeld op de aanwezigheid van uitbijters 
in de rapportagegrens of op kunstmatige trends. Op basis van een expert 
beoordeling (zie stap 4d) kunnen uitbijters en/of reeksen worden verwijderd.
...
Iteratie 1: < rapportagegrens wordt geherdefinieerd als de helft van de 
rapportagegrens.  
Iteratie 2: < rapportagegrens wordt geherdefinieerd als 0.
...
rate voor alle geselecteerde putfilters. Input is een vector met p-waardes van 
de Mann-Kendall statistiek. De laatste kolom van de output geeft aan of
de p-waardes significant zijn volgens de BH-criteria. Met de volgende 
code wordt de functie gebruikt:

```{r}
# Filter goedgekeurde reeksen
d <- grondwaterdata %>% left_join(putfilterselectie) %>% filter(afgekeurd == FALSE)
...
Iteratie 1 en 2 worden uitgevoerd om te bepalen of rapportagegrenzen (RG) van 
invloed zijn op het resultaat. Als de resultaten van iteratie 1 en 2 verschillen 
(bijvoorbeeld in iteratie 1 stijgend en in iteratie 2 dalend of geen trend), dan 
worden voor die reeksen een doorrekening van 10 jaar gedaan:  
-	Indien in de reeks metingen en/of na doorrekening 10 jaar >75% norm voorkomt, 
dan dient een visuele inspectie te worden uitgevoerd  
-	Indien dit niet het geval is, dan worden deze reeksen verder meegenomen alsof 
er ‘geen trend’ is.

In het geval dat een visuele inspectie moet worden uitgevoerd, moeten de 
resultaten van beide iteraties (RG = 0,5 RG en RG = 0) bekeken en vergeleken 
worden. o.b.v. een expert beoordeling wordt beoordeeld of 
de berekende trend een trend is in rapportagegrenzen of dat de trend wordt 
beïnvloed door uitbijters. Uitbijters kunnen worden verwijderd en de trend kan 
opnieuw worden berekend. Afhankelijk van de expert beoordeling wordt de trend 
meegenomen als ‘geen trend’ of als een ‘significante trend’. 

In 'Bijlage 1 van de Werkinstructie trendanalyse algemene grondwaterkwaliteit KRW' 
zijn een aantal voorbeelden gegeven van expert beoordeling van grafieken m.b.t. 
de invloed van rapportagegrenzen. 
...
### 4.e Verzamelen statistieken
  
Na het doorlopen van stappen 4b, 4c en 4d kan een tabel worden 
...
percentage benoemde dalende trends. In deze tabel worden ook de aantallen trends 
weergegeven die eventueel afvallen na visuele beoordeling in het eindresultaat. 
De trends worden berekend op de goedgekeurde reeksen.

Daarnaast wordt in deze tabel de informatie m.b.t. de percentages van de benoemde 
trends en de gemiddelde helling weergegeven van (1) de benoemde stijgende en 
dalende trends, (2) de benoemde stijgende trends, (3) de benoemde dalende trends, 
(4) de benoemde en niet-benoemde trends. De percentages van de benoemde trends 
worden weergegeven t.o.v. het totaal aantal reeksen en het aantal goedgekeurde 
reeksen. Voor het berekenen van deze percentages en de hellingen worden de trends 
genomen na visuele inspectie.
  
De resultaten van de trendtoets wordt opgenomen in tabel 6
(zie 'werkinstructie').
...
# Filteren goedgekeurde reeksen
d <- d %>% left_join(putfilterselectie) %>% filter(afgekeurd == FALSE)
...
In stap 5 wordt voor de stoffen die zijn geselecteerd in stap 2 onderzocht of 
trendomkering aanwezig is. Voor het detecteren van trendomkering 
...
toegelicht. Het statistisch toetsen van trendomkering is erg complex. De toets 
die nu gebruikt wordt, is makkelijk te gebruiken. Echter de toets moet vooral 
gezien worden als hulpmiddel. Na het toepassen van de toetsen en de correctie 
voor vals-positieven, moeten de reeksen waarin een trendomkering wordt benoemd 
daarom *altijd* visueel geïnspecteerd worden. 
...
te worden ingevuld in Tabel 7 uit de '*Werkinstructie*'.
