...
d <- grondwaterdata %>% 
  ...
  select(-data) %>% 
  unnest(tijdelijk) %>% 
  pivot_wider(names_from = tijdsperiode, values_from = c(aantal_metingen), values_fill = 0)
...
d <- grondwaterdata %>% 
  ...
  select(-data) %>% 
  unnest(tijdelijk) %>% 
  pivot_wider(names_from = tijdsperiode, values_from = c(voldoende_metingen), values_fill = 0)
