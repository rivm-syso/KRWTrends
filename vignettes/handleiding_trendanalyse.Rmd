...
uitspraak op niveau van een grondwaterlichaam. 
  ...
  e) *overzicht gekeurde reeksen* Voor het uitvoeren
...
# Standaard periodes
resultaat <- data.frame()
for(h in unique(grondwaterdata$grondwaterlichaam)) {
  
  for(i in unique(grondwaterdata$diepte)) {
    
    for(j in unique(grondwaterdata$parameter)) {
      
      for(k in unique(grondwaterdata$putfilter)) {
        
        d <- grondwaterdata %>%
          filter(grondwaterlichaam == h, diepte == i, parameter == j, putfilter == k) 
        
        resultaat <- resultaat %>%
          bind_rows(data.frame(grondwaterlichaam = h, diepte = i, parameter = j, putfilter = k,
                               meetverdeling(d), stringsAsFactors = FALSE)) 
  ...
  for(i in unique(grondwaterdata$diepte)) {
    ...
    for(j in unique(grondwaterdata$parameter)) {
      ...
      for(k in unique(grondwaterdata$putfilter)) {
        ...
        resultaat <- resultaat %>%
          bind_rows(data.frame(grondwaterlichaam = h, diepte = i, parameter = j, putfilter = k,
                               meetverdeling(d, jaren_overlap = 2), stringsAsFactors = FALSE)) 
...
##########################################################
# Waarnemingen boven rapportagegrens
##########################################################
# Aantal waarnemingen boven rapportagegrens
d <- grondwaterdata %>% 
